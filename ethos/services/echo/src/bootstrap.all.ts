...
import { processHourlyEthosJobs } from './contract-events/message-queue.js';
import { checkLatestScoreVersion } from './data/score/version.js';
import { startInternalEventsJob } from './internal-events/index.js';
...
if (cluster.isPrimary) {
  ...
  // Start the internal events job
  startInternalEventsJob().catch((err: unknown) => {
    logger.fatal({ err }, 'Failed to start internal events job');
    process.exit(1);
  });
  processHourlyEthosJobs().catch((err: unknown) => {
