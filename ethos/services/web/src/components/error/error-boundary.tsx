...
const ErrorMessage = ({ error, eventId, variant = 'large' }: ErrorMessageProps) => {
...
export const PageErrorBoundary: FallbackRender = (props) => {
...
export const ComponentErrorBoundary: FallbackRender = (props) => {
  const { showNewMessage } = useIntercom();
  const { token } = theme.useToken();
  const error = props.error;
