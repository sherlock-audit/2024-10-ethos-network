...
export function useSimulatedTransaction(
  ...
  return useQuery({
    ...
    queryKey:
      args.action === 'Buy'
        ? [SIMULATE_BUY_KEY, args.action, args.isPositive, args.funds.toString()]
        : [SIMULATE_SELL_KEY, args.profileId, args.isPositive, args.sellVotes],
    queryFn: async () => {
      let votes: bigint;
      let newPrice: bigint;
      let newFunds: bigint;
      try {
        if (args.action === 'Buy') {
          const { profileId, isPositive, funds } = args;
          ...
          const { profileId, isPositive, sellVotes, address } = args;
