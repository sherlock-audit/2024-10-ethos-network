...
contract ReputationMarket is AccessControl, UUPSUpgradeable, ReentrancyGuard {
  ...
  function _createMarket(
    ...
    if (msg.value < initialLiquidityRequired) revert InsufficientInitialLiquidity();
    ...
    )
  {
    ...
    if (fundsAvailable < votePrice) revert InsufficientFunds();
    ...
    )
  {
    ...
    if (votesAvailable < amount) revert InsufficientVotesOwned(profileId, msg.sender);
  ...
  function _checkProfileExists(uint256 profileId) private view {
    ...
    if (!exists || archived) revert InvalidProfileId();
  }
  ...
  function _checkSlippageLimit(
    ...
    if (actual < minimumVotes) revert SlippageLimitExceeded(actual, expected, slippageBasisPoints);
  }
