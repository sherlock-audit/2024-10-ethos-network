...
export type ReputationMarketError = InsufficientVotesOwnedError | InsufficientFundsError | Error;
...
export class ReputationMarket {
  ...
  async buyVotes(
    ...
    isPositive: boolean,
    value: bigint,
    maxVotes: number | bigint = 9999999999999999999999999999999n,
  ): Promise<ContractTransactionResponse> {
    return await this.contract.buyVotes(profileId, isPositive, maxVotes, { value });
  }
  ...
  async simulateBuy(
    ...
    try {
      return await this.contract.simulateBuy(profileId, isPositive, amount, funds);
    } catch (error) {
      const marketError = this.tryParseError(error);
      throw marketError;
    }
